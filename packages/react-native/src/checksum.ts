import type { MobileTerritoryChecksumAdapter } from "./types.js";
...
export function createMobileTerritoryChecksumAdapter(): MobileTerritoryChecksumAdapter {
  return {
    async sha256(bytes) {
...
export function sha256Hex(input: Uint8Array): string {
  const message = padSha256Message(input);
...
function padSha256Message(input: Uint8Array): Uint8Array {
  ...
  const bitLengthHigh = Math.floor(input.byteLength / 0x20000000) >>> 0;
