export interface TerritoryIsoCountryEntry {
  iso2: string;
  iso3: string;
  numeric: string;
  name: string;
...
// Seeded from ISO 3166-1 code pairs exposed by i18n-iso-countries 7.14.0 (MIT)
// and UNSD M49 English display names. XK is intentionally excluded because it is
// a user-assigned code, not an officially assigned ISO 3166-1 entry.
const ISO_3166_COUNTRY_TSV = `
AD|AND|020|Andorra
...
export const ISO_3166_COUNTRIES: readonly TerritoryIsoCountryEntry[] = ISO_3166_COUNTRY_TSV.split(
  "\n"
)
  .map((row) => {
    ...
    if (!iso2 || !iso3 || !numeric || !name) {
      throw new Error(`Invalid ISO 3166 country registry row '${row}'.`);
    }
...
export function getIso3166CountryEntry(countryCode: string): TerritoryIsoCountryEntry | undefined {
  const normalized = countryCode.trim().toUpperCase();

  return ISO_3166_COUNTRIES.find(
    (country) => country.iso2 === normalized || country.iso3 === normalized
  );
}
